...
TEST_CASE("blockades"){
        ...
        SECTION("Moved past home"){
                ...
                REQUIRE(mv2->inspect(rc, board) == Status::home_bonus);
        }
...
TEST_CASE("moving home"){
        ...
        Rules_Checker rc({8, 5, 6});

        mv_ptr mv{new MoveMain{board.final_ring[Color::red], 8, p0}};

        REQUIRE(mv->inspect(rc, board) == Status::home_bonus);

        board.move_pawn(board.final_ring[Color::red], 2, p0);

        mv_ptr mv1{new MoveHome{2, 6, p0}};

        REQUIRE(mv1->inspect(rc, board) == Status::home_bonus);
}
